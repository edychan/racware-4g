...
do while .t.
   ...
   case yptr = 10
      f_getfld (@ydate1, 22, 13, "W/N", 0, "99/99/99", .t.)
   ...
   case yptr = 11
      f_getfld (@ydate2, 22, 37, "W/N", 0, "99/99/99", .t.)
...
if yresvno = l_fresvno .or. empty (l_fresvno)
   return .t.
endif
f_use ("rares", 1)
set filter to fresvstat = "O"
seek l_fresvno

*if .not. f_valid (found () .and. fresvstat = "O", "Reservation Not Found!")
*   return .f.
*endif
...
elseif f_verify ("ravsm",1,l_funit)     && check for maint. due
   ...
   select ravm
   if ydue
      f_valid (.f., "Warning: This unit is due for maintenance; Code "+ravsm->fsvc)
      if f_confirm ("[R]etype  [C]ontinue  ", "RC") = "R"
         l_funit = space(10)
         return .f. 
      endif
...
*elseif fresv
*   if .not. f_validres (l_funit, l_fdateout, l_ftimeout, l_fdatein, ;
*         l_ftimein, "R" + l_fresvno)
*      return .f.
*   endif
*   f_markvres (l_funit, l_fdateout, l_ftimeout, l_fdatein, l_ftimein)
*else
*   f_relvres ()
endif
if fstatus = "I"
...
@ 18, 12 get l_fdateout valid f_valid (.not. empty (l_fdateout) .and. ;
             f_y2k (@l_fdateout))
@ 18, 21 get l_ftimeout picture [99:99] valid rrnoa6 ()
@ 19, 12 get l_fdays picture [999] valid rrnoa7 ()
@ 20, 12 get l_fdatein valid f_y2k (@l_fdatein) .and. rrnoa8 ()
@ 20, 21 get l_ftimein picture [99:99] valid f_valid (f_timeok (l_ftimein) ;
      .and. (dtos (l_fdatein) + l_ftimein) >= ;
      (dtos (l_fdateout) + l_ftimeout))
...
* 06/11/92 edc - retrieve insurance info. for customer acct.
